...
for y in range(0,len(wifi_change),2):
    ...
    if wifi_change[y] == -1:
        ...
        for x in range(len(wifi_change[y])):
            if wifi_change[y][x] != wifi_change[y+1][x]:
                wifi_change_new[y].append(wifi_change[y][x])
                wifi_change_new[y+1].append(wifi_change[y+1][x])
print(wifi_change_new) ##结果为-1的值表示无法判断 
print(len(wifi_change_new))   
...
with open('wifi_'+filename+'_new_content.csv','w', newline='') as f:##如果不加newline,存的文件里每隔一行会多一个空行
    ...
    for x in range(len(time_diff)):
        if wifi_change_new[x] == -1:
            f_csv.writerow([str(time_diff[x])] + [str(wifi_change_new[x])])
        else:
            f_csv.writerow([str(time_diff[x])] + wifi_change_new[x])
        if x%2 == 1:
            f_csv.writerow('\n')
    f_csv.writerow(['注释:-1表示无法判断;每两行为一组数据,同一组内去除了相同的wifi(即同时存在但数值相同或都不存在)'])
